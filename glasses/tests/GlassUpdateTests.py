...
class GlassUpdateTests(TestCase):
    ...
    def test_page_status_code_found(self):
        response = self.client.get(reverse("glass_update", args=(self.glass.id,)))
        self.assertEquals(response.status_code, 200)

    def test_page_status_code_not_found(self):
        response = self.client.get(reverse("glass_update", args=(1000,)))
